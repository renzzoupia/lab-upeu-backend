...
class Usuario extends Controller{
    ...
    public function create(){
        ...
        $validation = \Config\Services::validation();
        $datos = array(
            "usua_nombrecompleto" => $request -> getVar("usua_nombrecompleto"),
            "usua_username" => $request -> getVar("usua_username"),
            "usua_clave" => $request -> getVar("usua_clave"),
            "usua_role_id" => $request -> getVar("usua_role_id"),
            "usua_dni" => $request -> getVar("usua_dni"),
            //"usua_user_secreto" => $request -> getVar("usua_user_secreto"),
            //"usua_llave_secreta" => $request -> getVar("usua_llave_secreta")
        );
        if(!empty($datos)){
            $validation -> setRules([
                'usua_nombrecompleto' => 'required|string|max_length[255]',
                'usua_username' => 'required|string|max_length[50]',
                'usua_clave' => 'required|string|max_length[255]',
                'usua_role_id' => 'required|integer',
                'usua_dni' => 'required|integer',
            ...
            if($validation -> getErrors()){
                ...
                $usua_user_secreto = crypt($datos["usua_username"].$datos["usua_clave"], '$2a$07$dfhdfrexfhgdfhdferttgsad$');
	     		$usua_llave_secreta = crypt($datos["usua_clave"].$datos["usua_username"], '$2a$07$dfhdfrexfhgdfhdferttgsad$');
                $datos = array(
                    "usua_nombrecompleto" => $datos["usua_nombrecompleto"],
                    "usua_username" => $datos["usua_username"],
                    "usua_clave" => $datos["usua_clave"],
                    "usua_role_id" => $datos["usua_role_id"],
                    "usua_dni" => $datos["usua_dni"],
                ...
                $data = array(
                    ...
                    "credenciales" => array(
                        ...
                        "usua_llave_secreta" => str_replace('$','o',$usua_llave_secreta)
                    )
                );
                return json_encode($data, true);
            }
